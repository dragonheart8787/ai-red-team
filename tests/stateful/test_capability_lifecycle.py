...
D28 — restored, and what was reconsidered on the way back
---------------------------------------------------------
This file was deleted at `7c9295f` (D17), bundled into an unrelated commit, and
nobody noticed for three days because a deleted test does not fail — it stops
guarding. `ACCEPTANCE_MVP_KERNEL.md` §4.1 is the incident record. It is restored
here from `26ff2af`, the last revision before the deletion, **not** from D9's
original `86b18bb`: two later commits changed it, `8b6af07` adding the `event()`
branch instrumentation below and `26ff2af` moving `emergency_tighten` onto
`publish_policy_layer`. Restoring the older one would have quietly dropped both.

Every API the suite calls was re-checked against the current tree and none had
drifted, so the rules test what they always tested. One rule is new, and three
candidate rules were considered and deliberately left out:

* **Added — `expire_approval`.** §4.6's renewal checklist names policy, approval,
  credential, engagement and kill switch. The machine drove all of them except
  approval: capabilities were issued with `approval_id` NULL, so that branch had
  no interleaving coverage at all. Now seeded and exercised. See the rule's own
  docstring for the gap it exposes (there is still no `revoke_approval()`, and
  nothing cascades when an approval is withdrawn — deferred item 5.5).

* **Not added — a global policy-layer publish (D21).** `publish_policy_layer`
  can publish globally (`scoped_to_engagement=False`), and it is tempting to
  reach for that here. It would be wrong: a global layer applies to *every*
  engagement, so a state machine publishing them would silently perturb every
  other test sharing the database, and the failures would surface anywhere but
  here. `emergency_tighten` stays engagement-scoped for the same reason D9 gave
  originally — concurrent examples must stay independent. The global path is
  covered by `tests/test_global_audit.py` and `tests/test_policy_layers.py`,
  where the blast radius is one test's own fixtures.

* **Not added — `deactivate_policy_layer`.** Retiring a tightening overlay is a
  *widening*, and `i2_policy_only_tightens` asserts across the whole run that
  the effective `data_deny` never shrinks. Adding the operation would mean
  redefining what I2 checks here, and "restore the suite" is not the deliverable
  to change an invariant's meaning under. The operation is covered from a clean
  state in `tests/test_policy_layers.py`.

* **Not added — an uncascaded scope deactivation.** `deactivate_scope_object`
  retires a scope object *without* the eager cascade, which is the path the
  broker's lazy liveness check exists to backstop. Mutation testing during D28
  confirmed the machine cannot currently distinguish that check being present
  from absent, because `retire_scope_object` revokes eagerly and renewal never
  reaches the backstop. Left alone deliberately: `deactivate_scope_object` has
  exactly one caller in the whole control plane — `retire_scope_object` itself —
  so the uncascaded window is not reachable in production, and driving it here
  would manufacture an I1 violation the design has already closed. The lazy
  check is covered directly by four tests in `tests/test_capability_broker.py`.
"""
...
class CapabilityLifecycle(RuleBasedStateMachine):
    ...
    def setup(self):
        ...
        with engagement_scope(self.engagement_id) as conn:
            ...
            # D28: an approval for the capabilities to rest on. §4.6 lists
            # approval beside policy, credential, engagement and kill switch in
            # the renewal checklist, and until now the state machine exercised
            # every item on that list except this one — capabilities were issued
            # with approval_id NULL, so the branch never ran under interleaving.
            conn.execute(
                text("""
                    INSERT INTO approvals (approval_id, engagement_id,
                        action_class, resource, valid_until, approved_by,
                        approved_scope)
                    VALUES (:a, :e, :act, 'customer_network',
                            now() + interval '1 hour', 'user-X', 'this_task')
                """),
                {"a": self.approval_id, "e": self.engagement_id, "act": ACTION},
            )
    ...
    def issue(self, octet: int, which: str):
        ...
        expected_refusal = (
            self.killed or self.paused or self.credential_revoked
            or self.approval_expired
        )
        assert result.issued is not expected_refusal, (
            ...
            f"approval_expired={self.approval_expired}; reasons={result.reasons}"
        )
    ...
    @rule()
    @precondition(lambda self: not self.approval_expired)
    def expire_approval(self):
        """Let the §4.7 approval a capability rests on lapse (D28).

        The one item on §4.6's renewal checklist the D9 machine never exercised.
        ``check_preconditions`` has read ``approvals.valid_until`` and
        ``approvals.revoked`` since D5, and the unit tests cover both from a
        clean state — but nothing drove an approval expiring *partway through* a
        sequence, so the branch had no interleaving coverage at all.

        Written as raw SQL, and that is a finding rather than a shortcut. D9's
        own lesson was that driving a column instead of an operation proves the
        *check* works while saying nothing about whether the operation exists —
        which is how it discovered the credential cascade lived only in a
        docstring. The same hole is open here: expiry is a clock, but there is
        still no ``revoke_approval()`` anywhere in the control plane, so nothing
        cascades when an approval is withdrawn and every capability resting on it
        stays live until its next heartbeat refuses it. That is deferred item 5.5,
        and when the Approval API lands this rule should call it and the cascade
        should get the treatment ``revoke_credential()`` got in D9.

        Modelled as lazy, not eager, because that is what the system does: no
        capability is revoked at this moment, they simply stop renewing.
        """
        with engagement_scope(self.engagement_id) as conn:
            conn.execute(
                text("UPDATE approvals SET valid_until = now() - interval '1s' "
                     "WHERE approval_id = :a"),
                {"a": self.approval_id},
            )
        self.approval_expired = True

        outstanding = [
            capability_id for capability_id, state in self.issued.items()
            if not state["revoked"]
        ]
        for capability_id in outstanding:
            self.must_fail_renewal.add(capability_id)
        event(f"expire_approval: {len(outstanding)} capabilities now unrenewable")
